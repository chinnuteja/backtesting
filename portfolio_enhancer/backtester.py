...
from .kpis.nifty import analyze_nifty_composite  # <-- Equities (NIFTY) composite KPI
...
# KPI tilt strength (maps KPI score in [-1,1] to a multiplicative tilt 1±KPI_TILT_MAX)
KPI_TILT_MAX = 0.15

# ---- Output controls ----
# "full"   -> very verbose (like before)
# "summary"-> one-liners per period + tiny KPI line
# "quiet"  -> only final summary
PRINT_MODE = "summary"
SHOW_KPI_SUMMARY = True
SILENCE_KPI_LOGS = True           # hides internal prints from KPI modules
PRINT_LAST_N_MONTHS = 0           # 0 prints all months; otherwise only last N months

# KPI safety
KPI_TIMEOUT_SEC = 60              # defensive timeout per KPI call
...
async def _await_quietly(coro):
    """Capture prints from awaited KPI coroutine when SILENCE_KPI_LOGS is True."""
...
async def _safe_kpi(label: str, coro) -> Dict:
    """Timeout + exception-guarded KPI call. Returns neutral on failure."""
    try:
...
# =========================
# Optimizer helpers (Euclidean projection to capped simplex)
# =========================
def _project_capped_simplex(
    ...
    """
    Euclidean projection onto {x: sum x = 1, floors <= x <= caps}.
    Bisection on τ in x = clip(w_raw - τ, lo, hi) with final renorm.
    """
...
# KPI wrappers + tilt
# =========================
def _tilt_multiplier(score: float, max_tilt: float = KPI_TILT_MAX) -> float:
    """
    Map KPI score in [-1, 1] to multiplicative tilt in [1-max_tilt, 1+max_tilt].
    e.g. score=+1 -> 1+max_tilt; score=-1 -> 1-max_tilt
    """
    score = float(np.clip(score, -1.0, 1.0))
    return 1.0 + max_tilt * score

async def _equities_kpi(decision_date: pd.Timestamp, horizon_key: str) -> Dict:
    asof = decision_date.date().isoformat()
    return await _safe_kpi("EQ", analyze_nifty_composite(asof, _hletter(horizon_key)))
...
async def forecast_one_period(
    ...
    # KPIs — run sequentially (safer, silent, timeout-guarded)
    eq   = await _equities_kpi(decision_date, horizon_key)   # <-- NIFTY composite for Equities
    ...
    # -------- KPI tilt (hybrid model) --------
    s_eq   = float(eq.get("composite_sentiment", 0.0))
    s_gold = float(gold.get("composite_sentiment", 0.0))
    s_reit = float(reit.get("composite_sentiment", 0.0))
    s_btc  = float(btc.get("composite_sentiment", 0.0))

    tilts = np.array([
        _tilt_multiplier(s_eq),     # Equities (NIFTY composite)
        _tilt_multiplier(s_gold),   # Gold
        _tilt_multiplier(s_reit),   # REITs
        _tilt_multiplier(s_btc),    # Bitcoin
    ], dtype=float)

    mu_vec = mu_vec * tilts  # apply tilt to the trend forecast

    # Convert tilted mu into a long-only raw weight seed
    if (mu_vec > 0).any():
    ...
    kpi_pack = {"equities": eq, "bitcoin": btc, "gold": gold, "reit": reit}
